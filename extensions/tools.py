...
class ExpXboxController(object):
    MAX_TRIG_VAL = 1024
    MAX_JOY_VAL = 32768
        ...
        try:
            ...
            for event in self.gamepad.read_loop():
                ...
                if event.type == ecodes.EV_ABS:
                    ...
                    if event.code == 0:
                        self.LJoyX = self._clean(event.value / XboxController.MAX_JOY_VAL)  # normalize between -1 and 1
                    elif event.code == 1:
                        self.LJoyY = self._clean(
                            -(event.value / XboxController.MAX_JOY_VAL))  # normalize between -1 and 1
                    elif event.code == 2:
                        self.LT = self._clean(event.value / XboxController.MAX_TRIG_VAL)  # normalize between 0 and 1
                    elif event.code == 3:
                        self.RJoyX = self._clean(
                            event.value / XboxController.MAX_JOY_VAL)  # normalize between -1 and 1
                    elif event.code == 4:
                        self.RJoyY = self._clean(
                            -(event.value / XboxController.MAX_JOY_VAL))  # normalize between -1 and 1
                    elif event.code == 5:
                        self.RT = self._clean(event.value / XboxController.MAX_TRIG_VAL)  # normalize between 0 and 1
